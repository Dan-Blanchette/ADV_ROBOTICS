...
class Slash(Node):
    ...
    def __init__(self, namespace):
        ...
        self._undock_ac = ActionClient(
            self, Undock, f'/{namespace}/undock', callback_group=cb_Action)
        self._drive_ac = ActionClient(
            self, DriveDistance, f'/{namespace}/drive_distance', callback_group=cb_Action)
        self._rotate_ac = ActionClient(
            self, RotateAngle, f'/{namespace}/rotation_angle', callback_group=cb_Action)    
...
# --------------------Async send goal calls-----------------------------
    ...
    def sendRotationGoal(self, goal):
        ...
        with lock:
            rot_handle = self._rotate_ac.send_goal_async(goal)
            while not rot_handle.done():
                pass  # Wait for Action Server to accept goal

            # Hold ID in case we need to cancel it
            self._goal_uuid = rot_handle.result()
...
# ----------------------------------------------------------------------
    ...
    # 
        ...
        self._logger().warning('ROT_AC WAITING FOR SERVER')
        self._rotate_ac.wait_for_server()
        self.get_logger().warning('SERVER AVAILABLE')
        self.get_logger().warning('PREPARING TO ROTATE')
        ...
        self._logger().warning('DRIVER_AC WAITING FOR SERVER')
        self._drive_ac.wait_for_server()
        self.get_logger().warning('DRIVING!')
